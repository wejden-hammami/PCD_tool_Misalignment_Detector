...
from gerrit_client import get_latest_patchset, get_patch_diff, list_patchsets
...
def get_threshold() -> float:
    """Always returns the current slider value from session state."""
    return st.session_state.get("threshold", DEFAULT_THRESHOLD)


def invalidate_results_cache() -> None:
    """Clear all cached analysis results (e.g. when threshold changes)."""
...
def render_commit_card(item: dict, source: str = "github") -> None:
    ...
    if status == "MISALIGNED":
        if sugg:
            sugg_block = f'<div class="card-suggestion">Suggested: {sugg}</div>'
        else:
            sugg_block = (
        ...
        try:
            ...
            if status == "MISALIGNED":
                try:
                    ...
                    issue     = llm.get("issue", "")
                    suggested = llm.get("suggested_message", message)
                except Exception as llm_exc:
                    issue     = f"LLM analysis unavailable: {llm_exc}"
                    suggested = ""
            ...
            results.append({
                **commit,
                "diff": "", "score": 0.0, "status": "ERROR",
                "issue": str(exc), "suggested_message": "",
            })
...
with st.sidebar:
    ...
    if st.session_state.get("_last_threshold") != new_threshold:
        st.session_state["_last_threshold"] = new_threshold
